...
def make_predictions(config, PRED_START_DATE, model_file_path, prediction_quantile):
    ...
    tft = TemporalFusionTransformer.load_from_checkpoint(model_file_path)
    ...
    for x in config["large_time_windows"]:
        ...
        week_preds[f"target_date_1_year_{x}"] = week_preds[f"target_date_{x}"] - pd.DateOffset(years=1)

        # Create composite keys for initial and fallback target dates in week_preds
        week_preds[f"composite_key_{x}"] = week_preds[f"target_date_{x}"].astype(str) + "_" + week_preds["fueltype"]
    ...
    for x in config["large_time_windows"]:
        ...
        week_preds[f"shifted_1_year_{x}"] = week_preds[f"composite_key_1_year_{x}"].map(value_mappings[x])

        # Combine initial and fallback values, prioritizing initial where available
        week_preds[f"shifted_{x}"] = week_preds[f"shifted_{x}"].fillna(week_preds[f"shifted_1_year_{x}"])

    # Cleanup: drop intermediate columns
    columns_to_drop = [
        col for col in week_preds.columns if "target_date" in col or "composite_key" in col or "shifted_1_year" in col
    ]
